<div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
    ...
    <form wire:submit="save" class="space-y-4 max-w-2xl p-4 bg-surface-alt dark:bg-surface-dark-alt rounded-lg shadow-md">
        ...
        <x-form.input wire:model="identification" label="Identificación" name="identification" placeholder="Documento del proveedor"/>
        <x-form.input wire:model="name" label="Nombre del proveedor" name="name" placeholder="Ingresa el nombre del proveedor"/>
        <x-form.input wire:model="phone_number" label="Teléfono" name="phone_number" placeholder="Teléfono del proveedor"/>
        ...
        <div>
            ...
            @error('person_type_id') <p class="text-sm text-danger">{{ $message }}</p> @enderror
        </div>
